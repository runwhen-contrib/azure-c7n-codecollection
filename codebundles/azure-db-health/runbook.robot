...
List Publicly Accessible MySQL Flexible Servers in resource group `${AZURE_RESOURCE_GROUP}` in Subscription `${AZURE_SUBSCRIPTION_NAME}`
    ...
    ${report_data}=    RW.CLI.Run Cli
    ...    cmd=cat ${OUTPUT_DIR}/azure-c7n-db-health/mysqlfx-public-access/resources.json
    ...
    IF    len(@{server_list}) > 0
        ${formatted_results}=    RW.CLI.Run Cli
        ...    cmd=jq -r '["Server_Name", "Resource_Group", "Location", "Server_Link"], (.[] | [ .name, (.resourceGroup | ascii_downcase), (.location | gsub(" "; "_")), ("https://portal.azure.com/#@/resource" + .id + "/overview") ]) | @tsv' ${OUTPUT_DIR}/azure-c7n-db-health/mysqlfx-public-access/resources.json | column -t
...
List MySQL Flexible Servers Without Replication in resource group `${AZURE_RESOURCE_GROUP}` in Subscription `${AZURE_SUBSCRIPTION_NAME}`
    ...
    ${report_data}=    RW.CLI.Run Cli
    ...    cmd=cat ${OUTPUT_DIR}/azure-c7n-db-health/mysqlfx-no-replication/resources.json
    ...
    IF    len(@{server_list}) > 0
        ${formatted_results}=    RW.CLI.Run Cli
        ...    cmd=jq -r '["Server_Name", "Resource_Group", "Location", "Server_Link"], (.[] | [ .name, (.resourceGroup | ascii_downcase), (.location | gsub(" "; "_")), ("https://portal.azure.com/#@/resource" + .id + "/overview") ]) | @tsv' ${OUTPUT_DIR}/azure-c7n-db-health/mysqlfx-no-replication/resources.json | column -t
...
List MySQL Flexible Servers Without High Availability in resource group `${AZURE_RESOURCE_GROUP}` in Subscription `${AZURE_SUBSCRIPTION_NAME}`
    ...
    ${report_data}=    RW.CLI.Run Cli
    ...    cmd=cat ${OUTPUT_DIR}/azure-c7n-db-health/mysqlfx-ha-check/resources.json
    ...
    IF    len(@{server_list}) > 0
        ${formatted_results}=    RW.CLI.Run Cli
        ...    cmd=jq -r '["Server_Name", "Resource_Group", "Location", "Server_Link"], (.[] | [ .name, (.resourceGroup | ascii_downcase), (.location | gsub(" "; "_")), ("https://portal.azure.com/#@/resource" + .id + "/overview") ]) | @tsv' ${OUTPUT_DIR}/azure-c7n-db-health/mysqlfx-ha-check/resources.json | column -t
