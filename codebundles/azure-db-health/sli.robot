...
Check for MySQL Flexible Servers Without Replication in resource group `${AZURE_RESOURCE_GROUP}` in Subscription `${AZURE_SUBSCRIPTION_NAME}`
    ...
    ${no_replication_score}=    Evaluate    1 if int(${count.stdout}) <= int(${MAX_NO_REPLICATION}) else 0
    Set Global Variable    ${no_replication_score}


Generate Health Score
    ${health_score}=    Evaluate  (${public_mysql_score} + ${no_replication_score}) / 2
...
Suite Initialization
    ...
    ...    example=0
    ...    default=0
    ${MAX_NO_REPLICATION}=    RW.Core.Import User Variable    MAX_NO_REPLICATION
    ...    type=string
    ...
    Set Suite Variable    ${MAX_NO_REPLICATION}    ${MAX_NO_REPLICATION}